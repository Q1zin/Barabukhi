...
SCAN_TIME = 0.2  # seconds per scan burst (увеличено для лучшего обнаружения маяков)
POLL_FREQUENCY_HZ = 120  # fixed BLE scan rate (реалистичная частота с учетом времени сканирования)
...
def update_freq():
    ...
    if status == 200 and body:
        try:
            ...
            if "freq" in data:
                freq = data["freq"]
                print(f"dbg: Frequency updated: {body}")
    ...
    if status == 200 and body:
        try:
            ...
            if "write_road" in data:
                ...
                print(f"dbg: Write road status updated: {write_road}")
...
def send_signal_with_list(beacon_list):
    ...
    if status == 200 and body:
        ...
        print(f"Failed to send signal. Status: {status}")
        return None
...
def run_main_loop():
    ...
    scan_count = 0  # для отладки: сколько сканирований накоплено
    ...
    while True:
        try:
            ...
            if time_since_last_report >= report_period:
                ...
                if averaged:
                    ...
                    print(f"Sending {len(beacon_list)} beacons ({scan_count} scans over {time_since_last_report:.2f}s)")
                    send_signal_with_list(beacon_list)
                else:
                    print(f"No beacons detected ({scan_count} scans over {time_since_last_report:.2f}s)")
